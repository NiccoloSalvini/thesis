...
Given the structure of the webpage it is imposed an algorithmic search strategy gravitating around 2 main criteria: shortest paths and obligation of result. The algorithm starts considering a single session object argument, then It reads the inner HTML content in the session storing the information into a reusable check up obj `opensess`. Immediately after another object `price` is created and a CSS query is called on direct HTML nodes. The CSS location `.im-mainFeatures__title` points to a precise group of data right below the main title. Expectation are that price is a single element character vector, containing price and other unnecessary non-UTF characters. Then the algorithm enters into the first `if` statement. The logical conditions checks whether the object `price` the first search went lost. If it does not the algorithm jumps directly to the end of the function and returns a preprocessed quantity. As opposite If it does it considers again the reusable `opensess` and hits with a second css query `.im-features__value , .im-features__title`, pointing to second data location. Note that the whole search is done within the same session (i.e. using the same session object), so no more additional request information has to be sent). Since the latter CSS query points to data sequentially stored inside a list object, the newly initialized `price2` is a list containing various information. Then the algorithm flows into a second `if` statement that checks whether `"prezzo"` is matched in the list, if it does it returns the +1 position index element with respect to the "prezzo" position. This happens because data in price2 list are stored by couples sequentially, e.g. [title, "Appartamento Sempione", energy class, "G", "prezzo", 1200/al mese]. Ultimately the algorithm escapes in the else statement by setting `price2 = NA_Character_` once no CSS query is able to find data. the _NA_Character_ string type has to be imposed due to row bindind contraints.
...
Mettici lo *pseudocode* 
```{r genscraping, tidy=TRUE, linewidth = 70, echo=FALSE, eval=FALSE}
...
Web scraping have to naturally interact a vast number of times with both the client and server side as a result many precautions must be seriously taken. From the server side a scraper can forward as many requests as it could (as sessions) and as a consequence it might create a bottleneck of traffic impacting the overall server capacity. As a further side effect it can confuse the nature of traffic due to fake user agents provisioning and consequently analytics reports might be off track. 
Those are a portion of the reasons why most servers have their dedicated Robots.txt files. Robots.txt are _miss lit_ a way to kindly ask webbots, spiders, crawlers to access or not access certain parts of a webpage. The de facto ‘standard’ never made it beyond a _informal_ “Network Working Group INTERNET DRAFT”. Nonetheless, the use of robots.txt files is widespread due to the vast number of web scrawlers (e.g. https://en.wikipedia.org/robots.txt, https://www.google.com/robots.txt), and bots from Google, Yahoo adhere to the rules defined in robots.txt files, although their _interpretation_ might differ.

Robots.txt files [@robotstxt] essentially are plain text and always found at the root of a website's domain.  The syntax of the files in essence follows a field-name value scheme with optional preceding user-agent. Blocks are separated by blank lines and the omission of a user-agent field (which directly corresponds to the HTTP user-agent field) is seen as referring to all bots. Possible field names as pinpointed in [@google:robottxt] are: user-agent, disallow, allow, crawl-delay, sitemap, and host. 

Some standardized interpretation:
