...
```{r laplacevisual, fig.cap="Chisquared density function with parameter $k = 8$ (top) and $k = 16$ (down) solid line. The point line refers to the corresponding Normal approximation obtained using the Laplace method"}
set.seed(2021)
df1 = 8 
df2 = 16
...
sigma2_star1 = 2*(df1 - 2)
sigma2_star2 = 2*(df2 - 2)
...
chisq8 = dat %>% 
 ggplot(aes(x = x)) +
    ...
    stat_function(fun = dnorm, args = list(mean =x_star1, sd =sqrt(sigma2_star1)), geom = "point") +
  xlim(0,30)+
  xlab(TeX("$k \\,=\\,  8$")) +
  ylab("Density")+ 
  geom_vline(xintercept = alpha1, alpha = 0.6, linetype = "dotdash",color = "red")+
  geom_vline(xintercept = beta1,alpha = 0.6, linetype = "dotdash",color = "red")+
  annotate("text", x = 20, y = 0.05, label = TeX("$\\int_{5}^{7} \\chi^{2}(\\, 8\\,) \\mathrm{d} x\\, = \\, 0.2209435 $"),size = 4) +
  annotate("text", x = 20, y = 0.09, label = TeX("$\\int_{5}^{7} \\mathit{N}(\\, 6,\\, 12) \\mathrm{d} x\\, = \\, 0.2209686 $"),size = 4) +
  theme_nicco()

chisq16 = dat %>% 
 ggplot(aes(x = x)) +
    stat_function(fun = dchisq, args = list(df = df2)) +
    stat_function(fun = dnorm, args = list(mean =x_star2, sd =sqrt(sigma2_star2)), geom = "point") +
  xlim(0,50)+
  xlab(TeX("$k \\,=\\,  16$")) +
  geom_vline(xintercept = alpha2, alpha = 0.6, linetype = "dotdash",color = "red")+
  geom_vline(xintercept = beta2,alpha = 0.6, linetype = "dotdash",color = "red")+
  annotate("text", x = 35, y = 0.03, label = TeX("$\\int_{12}^{17} \\chi^{2}(\\, 16\\,) \\mathrm{d} x\\, = \\, 0.3583827 $"),size = 4) +
  annotate("text", x = 35, y = 0.062, label = TeX("$\\int_{12}^{17} \\mathit{N}(\\, 14,\\, 28) \\mathrm{d} x\\, = \\, 0.3576164 $"),size = 4) +
  ylab("Density")+ 
  theme_nicco()
...
# 
# ## exact values
# pchisq(beta1, df = df1) -pchisq(alpha1, df = df1)
# pchisq(beta2, df = df2) -pchisq(alpha2, df = df2)
# ## approx values
# dchisq(x_star1, df = df1)*
#   sqrt(2*pi*sigma2_star1)*
#   (pnorm(beta1,mean=x_star1,sd=sqrt(sigma2_star1)) -
#   pnorm(alpha1,mean=x_star1,sd=sqrt(sigma2_star1)))
# 
# dchisq(x_star2, df = df2)*
#   sqrt(2*pi*sigma2_star2)*
#   (pnorm(beta2,mean=x_star2,sd=sqrt(sigma2_star2)) -
#   pnorm(alpha2,mean=x_star2,sd=sqrt(sigma2_star2)))

# 
# xx=seq(0,20,l=100)
# plot(xx,dchisq(xx, df = df1),t="l",ylab="Density",xlab="x")
# par(new=TRUE)
# plot(xx,dnorm(xx,mean=x_star1,sd=sqrt(sigma2_star1)),lty=2)
```
