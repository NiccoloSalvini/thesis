...
stack_fin <- inla.stack(
  ...
  effects = list(
    ...
    final_data2 %>%
      ...
      transmute(Intercept = 1,condom, totlocali,altro, bagno, cucina, heating,
                photosnum, floor, sqfeet, fibra_ottica, videocitofono, impianto_di_allarme,
                reception, porta_blindata ) ## fixed effect covariates
  )
)
...
Since the models coefficients are many i.e.`r dim(summ_fin)[1]` the analysis will concentrate on the most interesting ones. Looking at TAB(...) arranged by descending mean, the posterior mean for the intercept, rescaled back from log, is actually quite lofty, affirming that the average house price is `r paste(round(exp(summ_fin[1,2]),2),"€", sep = " ")`.
...
```{r summ_hyper, fig.cap="Marginal Hyper Parameter distributions for each element of Psi"}
plot_sigma2 = ggplot(as.data.frame(model_final$marginals.hyperpar$`Precision for the Gaussian observations` )) +
  geom_line(aes(x = x, y = y)) +
  ylab (TeX("$\\pi(\\,\\sigma^2_{e} \\,|\\, \\mathbf{y}\\,)$")) + 
  xlab (TeX("$\\sigma^2_{e}$")) +
  theme_nicco()

plot_kappa = ggplot(as.data.frame(model_final$marginals.hyperpar$`Theta1 for site` )) +
  geom_line(aes(x = x, y = y)) +
...
plot_nu = ggplot(as.data.frame(model_final$marginals.hyperpar$`Theta2 for site` )) +
  geom_line(aes(x = x, y = y)) +
  ylab (TeX("$\\pi(\\,\\nu \\,|\\, \\mathbf{y}\\,)$")) + 
  xlab (TeX("$\\nu$")) +
  theme_nicco()
...
```{r pred_on_grid, fig.cap="Prediction on predictive posterior mean a 122X122 grid overlapped with the Mesh3"}
...
model_grid <- inla(log(price) ~ 0 + Intercept + X + Y + f(site, model = spde3), 
                   ...
                   control.mode = list(theta = model_final$mode$theta, 
                                       restart = FALSE),
