...
When $r = 1$ it is a simple stochastic process widely explored in literature i.e. time series process. Nevertheless geostatistical data belong always to $r = 2$ or eventually $r = 3$, when elevation dataa is available. The stochastic process $Y$ is observed in a fixed set of "monitoring stations" and inference can be done regarding moments of the realized process. This information are essential to build a spatially continuous surface over the y-studied variable in order to predict the phenomenon at locations not yet observed.  

![3D scatterplot and surface, Stockton data.](images/prdprocess.png)

A $Y$ random spatial process is said to be a **GP** if for any set of spatial index $n$ and for each set of corresponding locations $\left(y\left(s_{1}\right), \ldots, y\left(s_{n}\right)\right)$ follows a multivariate _Gaussian_ distribution with mean $\boldsymbol{\mu}=\left(\mu\left(\boldsymbol{s}_{1}\right), \ldots, \mu\left(\boldsymbol{s}_{n}\right)\right)$ and covariance matrix $\mathbf{\Sigma}$ [@Blangiardo-Cameletti]. The covariance matrix relates each observation to each others by a spatial covariance function defined as $\mathcal{C}(\cdot)$, such that $\Sigma_{i j}=\operatorname{Cov}\left(y\left(s_{i}\right), y\left(s_{j}\right)\right)=\mathcal{C}\left(y\left(s_{i}\right), y\left(s_{j}\right)\right)$. 
GP has to meet two important properties: it has to be **stationary** and it has to be **isotropic**. 
...
where $N(t)$ is the set of location pairs such that $\left\|\mathbf{s}_{i}-\mathbf{s}_{j}\right\|=t$ and so $|N(t)|$ is the number of pairs in the set. As already guessed before empirical semivariogram values are expected to be small at short pairs distance and tends to increase when distance increases. The rational behind is that similar observations are expected to lay close together (small $h$) leading to lower semivariogram values ($\gamma(\mathbf{h})$), as opposite farther pairs obervations (big $h$) tend to be different and associated to greater semivariogram values. Flat semivariogram might indicate small spatial variance, since whether distance $h$ increases or not, semivariogram values remains the same $\frac{\Delta\gamma(\mathbf{h})}{\Delta\mathbf{h}}\approx0$. Semivariograms might be also implied earlier in the modeling process to to evaluate the presence of any spatial pattern. Then They are implied also later in the final stage of modelling process with the aim to asses if any spatial pattern in still present in the residuals. The three main arguments in semivariograms are: _range_ $\sigma^{2}$, _nugget_ $\tau^{2}$ and _sill_ $\tau^{2}+\sigma^{2}$. [parameters explanation]

### Matérn covariance function{#Matern}

There are many covariance functions and their application depends both on data and the scope on the analysis. Examples of spatial covariance functions are: Linear, Spherical, Exponential and Matérn. FIgure below shows the amount of differen covariance function
Matérn displays higher degree of flexibility and it can be accounted as the most used in geostatistics [@Krainski2018] and [@Bayesian_INLA_Rubio], therefore it will be implied in later the model building process. Matérn functional is: 

$$
C_{\nu}(d)=\sigma^{2} \frac{2^{1-\nu}}{\Gamma(\nu)}\left(\sqrt{2 \nu} \frac{d}{\rho}\right)^{\nu} K_{\nu}\left(\sqrt{2 \nu} \frac{d}{\rho}\right)
$$
where $d$ is the between point distance, $\Gamma(\cdot)$ is a gamma function and $K_{\nu}(\cdot)$ is a modified Bessel function $\sigma^2$ $\rho$ and $\nu$ are common shared parameters explained few lines below. 
...
## Hierarchical regression for univariate spatial data 

Since the relationships between the characteristics of the house, i.e. vector $C$, and the price is not defined in economic literature it is possible to assume any $f$ functional form [linear non linear functional]. The open possibility to apply any relationship between covariates comes very handy in the INLA setting \ref(latentGaussian) since latent gaussian models are prepared to accept any $f$ relation. Urban econometric literature transform the price variable to a logarithmic scale, this transformation smooths the skewness of prices leading to best estimates. This argumentation can be taken also for prices regarding house rents.
...
When leveraging information from location the added $w(\mathbf{s})$ has to be introduced in the previous equation \@ref(eq:genregvec). $w(\mathbf{s})$ surface is required in order to incroporate spartial infromation into regression settings, as well as enabling prediction over the domanin $D$:

$$y\left(\mathbf{s}_{i}\right)=\mathbf{x}\left(\mathbf{s}_{i}\right)^{\prime}\beta_{j}+w(\mathbf{s})+\varepsilon\left(\mathbf{s}_{i}\right)$$

The $w(\mathbf{s})$ in the context of the analysis is approached as a GP\@ref(GP) whose distribution is multivariate with mean $\boldsymbol{\mu}(\mathbf{s})$ and function of the spatial index $\mathbf{s}$ and covariance matrix $\mathbf{\Sigma}$.
$\varepsilon(\mathbf{s})$ is iid and mean centered in 0 with variance $\tau^{2}$ and is called non-spatial error since it contributes to the nugget. The error term is nit pure since it interfers with the covariance function so the model can be fully spatial. 
One of the major advantges of having a a spatial process embedded into a GP is likelihood based inference. 
