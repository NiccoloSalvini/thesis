...
REST API seen in chapter \@ref(Infrastructure) extracts point referenced data, so modeling methodologies described in this analysis will exclusively take into account point referenced techniques.
In order to extend the notion from discrete measurements (i.e. point referenced) to a continuous spatial surface a stochastic process, namely Gaussian Process, has to be introduced and constrained according to convenient properties. GP are then blended with a specific covariance function, i.e. Matèrn. The reason why last assumption is true will be much more clear in the next chapter \@ref(spde). 
A class of models called Hedonic Price are borrowed from econometric literature and are used to fully integrate covariates coming from the API. Then Hedonic models are exploited into a bayesian hierarchical framework and INLA is applied as computing choice. 
Spatial kriging is essential to predict the process at new locations so that the surface can be plotted and analyzed. 
In the end models have to be checked and verified with sanpling techiniques which are once again specific to the data type and the scope of the analysis.



_(forse mettere alla fine come further developments)_
As a side note Spatial data can also be measured according to a 3rd (or even a 4th in the case elevation data) dimension which is the time component. In this setting it might be interesting to research time correlation between subsequent spatial data time points, a valuable reference is @PACI2017149. Therefore since API generates data on a daily basis and then stores a .json file on a DB, future research on this data might consider the idea to include the time component in the model. 
...
(metti expressione variogram).

The process is said to be **Isotropic** if the covariance function depends only on the between-points distance so it is invariant under _rotation_ [-@Krainski-Rubio]. A further way of seeing the property is that Isotropy implies concentric decaying contours that resemble the vanishing of spatial dependence. 

![isotropy VS anisotropy, source @blanchetscalliet](images/isotropyVSanisotropy.png)

the If the last assumption does not hold and direction towards point are distant from each other in the spatial domain $D$ then the process is said to be **Anisotropic**.


$$C(\mathbf{s}+\mathbf{h}, \mathbf{s} \mid \theta)=\sigma^{2} \mathbf{R}(\|h\| ; \phi)$$

Once 
The three main arguments in semivariograms are: _range_ $\sigma^{2}$, _nugget_ $\tau^{2}$ and _sill_ $\tau^{2}+\sigma^{2}$. 
...
There are many covariance functions and their application depend both on data and the scope on the analysis.
Examples of spatial covariance functions among all the others are: Linear, Spherical, Exponential and Matérn.
Matérn is special since when it is used together with a GP, the SPDE approach can provide a GMRF representation of the same process, chapter \@ref(spde) undisclose this fundamental property.
Matérn can also be accounted as the most used in geostatistics [@Krainski2018] and [@Bayesian_INLA_Rubio]. Matérn is tuned mainly by two parameters, a scaling one $\kappa>0$ (the most of the times set equal to the range) and a smoothing one $\nu>0$. By decomposing it into a more granular form and cosidering two generic locations $\mathbf{S}_{i}$  and $\mathbf{S}_{j}$,  a _stationary_ and _isotropic_ Matérn _correlation_ function [@Krainski2018] has this form
...
If it assumed a log-linear model then a classical bayesian non-spatial linear regression can relate a $log(y)$ response variable with its covariates and error through the expression:
...
where the $i$th pedix is the observation index and the $j$th pedix is the mth covariate.  Moreover it is possible to specify $\eta$ as a function of $\mu$. The link function between them in this case is identity, so that $\eta_{i}=g\left(\mu_{i}\right)=\mu_{i}$. In GLMs when response variable has to stay between $[0,1]$ (e.g. probabilities), the link function might be Logit, which leads to logistic regression. More generally expressed in vector notation:
