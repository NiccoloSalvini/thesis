...
\CommentTok{#* @param n the number of samples}
\CommentTok{#* @serializer png}
\CommentTok{#* @get /plot}
\ControlFlowTok{function}\NormalTok{(}\DataTypeTok{n =} \DecValTok{10}\NormalTok{) \{}
\NormalTok{  rand =}\StringTok{ }\KeywordTok{rnorm}\NormalTok{(}\DataTypeTok{n =}\NormalTok{ n)}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{tipo =}\StringTok{ }\KeywordTok{tolower}\NormalTok{(type) }\OperatorTok{%>%}\StringTok{ }\KeywordTok{str_trim}\NormalTok{()}
\NormalTok{citta =}\StringTok{ }\KeywordTok{tolower}\NormalTok{(city) }\OperatorTok{%>%}\StringTok{ }\KeywordTok{iconv}\NormalTok{(}\DataTypeTok{to=}\StringTok{'ASCII//TRANSLIT'}\NormalTok{) }\OperatorTok{%>%}\StringTok{  }\KeywordTok{str_trim}\NormalTok{()}
\NormalTok{macroozone =}\StringTok{ }\KeywordTok{tolower}\NormalTok{(macrozone) }\OperatorTok{%>%}\StringTok{ }\KeywordTok{iconv}\NormalTok{(}\DataTypeTok{to=}\StringTok{'ASCII//TRANSLIT'}\NormalTok{) }\OperatorTok{%>%}\StringTok{  }\KeywordTok{str_trim}\NormalTok{()}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
 \ControlFlowTok{if}\NormalTok{ (npages }\OperatorTok{>}\StringTok{ }\DecValTok{300} \OperatorTok{&}\StringTok{ }\NormalTok{npages }\OperatorTok{>}\StringTok{ }\DecValTok{0}\NormalTok{)\{}
\NormalTok{                        msg =}\StringTok{ "npages should lay inbetween 1 and 300"}
\NormalTok{                        res}\OperatorTok{$}\NormalTok{status =}\StringTok{ }\DecValTok{500} \CommentTok{# code status: Bad request}
                        \KeywordTok{stop}\NormalTok{(}\KeywordTok{list}\NormalTok{(}\DataTypeTok{error=}\NormalTok{jsonlite}\OperatorTok{::}\KeywordTok{unbox}\NormalTok{(msg)))     }
\NormalTok{            \}}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{                req}\OperatorTok{$}\NormalTok{HTTP_USER_AGENT, }\StringTok{"@"}\NormalTok{, }
\NormalTok{                req}\OperatorTok{$}\NormalTok{REMOTE_ADDR, }\StringTok{"}\CharTok{\textbackslash{}n}\StringTok{"}\NormalTok{,}
\NormalTok{                req}\OperatorTok{$}\NormalTok{QUERY_STRING, }\StringTok{"}\CharTok{\textbackslash{}n}\StringTok{"}\NormalTok{)}
\NormalTok{            plumber}\OperatorTok{::}\KeywordTok{forward}\NormalTok{()}
\NormalTok{\}}
\end{Highlighting}
\end{Shaded}

The above filter parses the request through the default request argument \texttt{req}, then it prints out messages about the incoming User Agent (i.e.~\texttt{HTTP\_USER\_AGENT}) (section \ref{spoofing}), the \texttt{REMOTE\_ADDR} which is the IP address of the client making the request \citeyearpar{plumber} and the \texttt{QUERY\_STRING} that records the parameters directly sent the endpoint. This helps to traceback clients activity on the API as well as detecting misuse.
...
\begin{figure}
\centering
\includegraphics{images/dockerfile.jpg}
...
\begin{itemize}
\item
  \texttt{FROM\ rocker/tidyverse:latest} : The command imports from rocker project a pre set config guration containing the latest version of base-R along with the tidyverse \citep{tidyverse} R packages collection.
\item
...
\item
  \texttt{RUN\ apt-get\ update\ \&\&\ apt-get\ install\ -y\ \textbackslash{}\ libxml2-dev\ \textbackslash{}\ libudunits2-dev} : The command update and install remaining ``lower'' Linux dependencies required to run Plumber and rvest.
\item
  \texttt{RUN\ R\ -e\ "install.packages(c(\textquotesingle{}tictoc\textquotesingle{},\textquotesingle{}here\textquotesingle{},\textquotesingle{}...\textquotesingle{},dependencies=TRUE)} : The command install all the further libraries required to execute the files. Since some packages have related dependencies the option \texttt{dependencies=TRUE} is needed.
\item
...
\item
  \texttt{COPY\ \textbackslash{}\textbackslash{}} : The command tells Docker engine to copy all the files in the container as is.
\item
  \texttt{EXPOSE\ 8000} : The commands instructs Docker engine that the container listens on the specified network ports 8000 at runtime. Default \emph{transportation} layer is TCP.
\item
  \texttt{ENTRYPOINT\ {[}"Rscript",\ "main.R"{]}} : the command tells docker engine to execute the file main.R where are contained the Plumber router options (i.e.~host and port specifications).
\end{itemize}
