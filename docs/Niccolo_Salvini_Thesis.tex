...
\begin{figure}
\centering
\includegraphics{06-exploratory_files/figure-latex/semivariogram-1.pdf}
\caption{\label{fig:semivariogram-1}Semivariogram on a linear model Pearson residuals}
\end{figure}
...
\begin{figure}
\centering
\includegraphics{06-exploratory_files/figure-latex/semivariogram-2.pdf}
\caption{\label{fig:semivariogram-2}Semivariogram on a linear model Pearson residuals}
\end{figure}
...
Since the models coefficients are many i.e.66 the analysis will concentrate on the most interesting ones. Looking at TAB(\ldots) arranged by descending mean, the posterior mean for the intercept, rescaled back from log, is actually quite lofty, affirming that the average house price is 235.1 €.
...
INLA is able to output the mean and the standard deviation of measurement precision error in \(\psi_2\) i.e.~\(1 / \sigma_{\varepsilon}^{2}\), note that the interpretation is different from variance. In figure @ref(fig:summ\_hyper) are plotted the respective marginal hyper parameter distributions for \(\boldsymbol\psi\).
...
\begin{figure}
\centering
\includegraphics{07-model_fitting_files/figure-latex/summ_hyper-1.pdf}
\caption{(\#fig:summ\_hyper)Marginal Hyper Parameter distributions for each element of Psi}
\end{figure}
...
\begin{figure}
\centering
\includegraphics{07-model_fitting_files/figure-latex/pltgmrf-1.pdf}
...
\begin{figure}
\centering
\includegraphics{07-model_fitting_files/figure-latex/modelcpo-1.pdf}
...
Higher prices in @ref(fig:pred\_on\_grid) are observed nearby the points of interest in \ref{ggmap}, however the north-ovest displays lower monthly prices. The variance is ostensibly considerable within the domain and unsurprisingly decreases where data is more dense.

\begin{figure}
\centering
\includegraphics{07-model_fitting_files/figure-latex/pred_on_grid-1.pdf}
\caption{(\#fig:pred\_on\_grid)Prediction on predictive posterior mean a 122X122 grid overlapped with the Mesh3}
\end{figure}
