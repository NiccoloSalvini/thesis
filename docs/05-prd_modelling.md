...
![(#fig:isoovsanis)Left: isotropical concentric decaying contours, Right: anisotropical concentric decaying contours, source @blanchetscalliet](images/isotropyVSanisotropy.png)

In spatial statistics the isotropical assumption is very frequent despite being restrictive for describing the rich variety of interactions that can characterize spatial processes. Anyway assuming the property it offers a wide range of underlying functions that can model spatial dependence for which three are the most common ones [@Krainski2018], where all the parameters below are special quantities derived from the emphirical covariance function, where $\phi^2$, $\tau^2$ and $\sigma^2$ are respectively the range, the distance at which correlation vanishes, the nugget, i.e. the non spatial variance and the partial sill, i.e. the spatial effect variance [@LecturePaci].
...
In the end summarizing the result obtained in chapter \@ref(inla) with what it has been seen so far the model assumes the following mesaurament equation, where $\xi_{i}$ is the spatial random process, $\varepsilon_{t} \sim N\left(\mathbf{0}, \sigma_{\varepsilon}^{2} I_{d}\right)$ is the white noise error and $I_{d}$ is an identity matrix.
