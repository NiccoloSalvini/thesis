...
## R-INLA package in a bayesian hierarchical regression perspective

### Overview

INLA computations and methodology is developed by the R-INLA project whose package is available on their [website](http://www.r-inla.org). Download is not on CRAN (the Comprehensive R Archive Network) so a special source repo link, which is maintained by authors and collaborators, has to be optioned. The website offers also a forum where a daily discussion group is opened and an active community is keen to answer. Moreover It also contains a number of reference books, whose some of them are open sourced as gitbook. Furthermore the project is gaining importance in the bayesian world due to its recent applications and use cases. 
The core function of the package is `inla()`and it works as many other regression functions like `glm()`, `lm()` or `gam()`. Inla function takes as arguments the formula (where response and linear predictor are) together with the data (expects a data.frame obj) on which estimation is desired. Many other alternative methods inside the function can be added with respect to the model to be imposed. Inla objects are inla.dataframe summary-type lists containing the results from model fitting. Results contained in the object are specified in the table below, even though some of them requires special method: (meglio in kable)
...
Data is generated from a Gaussian variable and its moments are $\mu_{i}$ and precision $\tau$. 
The mean moment for the gaussian distribution $\mu_{i}$  has to be specified as _linear predictor_ $\eta_{i}$ (i.e. $E\left(y_{i} \mid \beta_{0}, \ldots, \beta_{M}, x_{i 1}, \ldots, x_{i M}\right) = \eta_{i}$ ). The link function is identity in section \@ref(genregvec) is  $\eta_{i}=\beta_{0}+\sum_{m=1}^{M} \beta_{m} x_{m i}+\sum_{l=1}^{L} f_{l}\left(z_{l i}\right)$. 
...
The summary of the model parameters already described is:
