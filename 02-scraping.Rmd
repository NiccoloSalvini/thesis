...
library(furrr, warn.conflicts = F, quietly = F)
library(robotstxt, warn.conflicts = F, quietly = F)
library(magrittr, warn.conflicts = F, quietly = F)
library(rvest, warn.conflicts = F, quietly = F)
library(parallel, warn.conflicts = F, quietly = F)
library(memoise, warn.conflicts = F, quietly = F)
library(knitr, warn.conflicts = F, quietly = F)
library(glue, warn.conflicts = F, quietly = F)
library(purrr, warn.conflicts = F, quietly = F)
```
...
Web Scraping is a technique aimed at extracting unstructured data from static or dynamic internet web pages and collecting it in a structured way. It can be applied simultaneously or automatically by a scheduler that plans the execution at a given time.
```
...
```{r scrapeallinfo, tidy = FALSE, linewidth = 70, eval = FALSE, echo = FALSE, size="tiny"}
## [ fastscrape ] ----
...
```{r permissions, echo=FALSE, eval=FALSE}
## at the domain 
...
```{r possibly, eval=FALSE, echo=FALSE}
price =  future_map(npages_vec, 
                    .f = possibly( ~{
                      sesh = html_session(.x, user_agent(agent = agents[sample(1)]))
                      scrapeprice_imm(session = sesh)},
                      otherwise = NA_character_,
                      quiet = FALSE)) %>% flatten_chr()
...
```{definition, parallel, name = "parallel", echo = TRUE}
_Parallel execution_ is characterized as multiple operations taking place over overlapping time periods. [@eddelbuettel2020parallel]
```
