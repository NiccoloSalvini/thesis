...
The user Agent (from now on refered as UA) "retrieves, renders and facilitates end-user interaction with Web content" [@UaDef].
```
...
```{r defaultUA, echo=FALSE}
prova = "http://httpbin.org"
html_session(prova)$response$request$options$useragent
```
...
```{definition, proxy, name="Proxy Server", echo =  TRUE}
A proxy server is a gateway between the web user and the web server.
```
