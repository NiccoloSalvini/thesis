...
Functions are esplicitely designed to respond to continuos changes and to handle fails. They need to be dynamic otherwise they would not get the content. Since none can predict whther a system administrator will change the page layout, the scrapping is disassembled into x number of functions. The number x of funvtions nearly corresponds to the final number of covariates ezxtracted (some of the functione extracts more than one covariates). In such a way the maintainer can easily adjust the failing function by accessing through it. 
