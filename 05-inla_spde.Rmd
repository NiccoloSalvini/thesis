...
where $\Phi(⋅)$ denotes the cumulative density function of th $Normal(x_i, \sigma^{2*})$ distri-
bution.
...
Here 𝛽0 is a scalar representing the intercept; the coefficients $\boldsymbol{\beta}=\left\{\beta_{1}, \ldots, \beta_{M}\right\}$
...
$f=\left\{f_{1}(\cdot), \ldots, f_{L}(\cdot)\right\}$ is a collection of functions defined in terms of a set of covariates $z = (z_1, … , z_L)$. The terms $f_l(⋅)$ can assume different forms such as smooth and
...
in a set of parameters named 𝜽 defined as $\boldsymbol{\theta}=\left\{\beta_{0}, \boldsymbol{\beta}, \boldsymbol{f}\right\}$. Moreover, we denote with
...
where each data point yi is connected to only one element 𝜃i in the latent field $\theta$.
...
We assume a multivariate Normal prior on 𝜽 with mean 𝟎 and precision matrix
...
where | ⋅ | denotes the matrix determinant and ′ is used for the transpose operation.
...
The joint posterior distribution of 𝜽 and $\psi$ is given by the product of the likelihood
...
as the SPDE representation. In such cases each observation $y_{i}$ depends on a linear combination of the elements of 𝜽 and the observation distribution will be defined as:
