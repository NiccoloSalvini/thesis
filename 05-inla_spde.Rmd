...
INLA uses a combination of analytics approximations and numerical integration to obtain an approximated posterior distibution of the parameters much faster.
...
INLA is not going yto try to estiamte posterior distribution, instead marginals effects and hyperparameters. For latent parameter $\boldsymbol{\theta}$ :
...
$$
where $\psi_{-k}$ is a vector of hyper paramenters $\psi$ without element $\psi_{k}$.
