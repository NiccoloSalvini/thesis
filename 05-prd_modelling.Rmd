...
```{r matern, echo=FALSE, message=FALSE, warning=FALSE, eval=FALSE}
library(geoR)

## Matern 

curve(matern(x, phi= 0.25, kappa = 1),
      from = 0,
      to = 1.5,
      xlab = "distance", 
      ylab = "covariance",
...
par(new=TRUE)
curve(matern(x,phi= 0.25, kappa = 1/2), ## expontential
      from = 0,
      to = 1.5, 
      add = TRUE)
par(new=TRUE)
curve(matern(x, phi= 0.25, kappa = 3/2), ## conveninet closed form 
      from = 0,
      to = 1.5, 
      add = TRUE,
      lwd = 2, 
      lty=2)
par(new=TRUE)
curve(matern(x, phi= 0.25, kappa = 80), ## gaussian 
      from = 0,
      to = 1.5, 
      add = TRUE,
      lwd = 2)
par(new=TRUE)
legend("topright", 
       expression(kappa==1,kappa==1/2, kappa==3/2, kappa==100),
       lty=c(2,1,2,1), 
       lwd=c(1,1,2,2))
...
# x = seq(1,20)
# ggplot(tibble(x = x))+
#   stat_function(fun = cMatern, args = list(nu = 0.25, kappa = 100))
...
Since INLA can fit a wide range of models then the flexibility should be reflected by a mouldable tool to check model suitability. 
Once the model is set up a resampling scheme has to be imposed in order to evaluate the model performance. One of the most used method to assess beyasian model quality is LOOCV cross validation and defualt choice fo R-INLA package. From data is left out one single observation and so that the Validation set is  $\boldsymbol{y}_{v} = \boldsymbol{y}_{-i}$ and the Assessement set is a $\boldsymbol{y}_{a} = \boldsymbol{y}_{i}$
