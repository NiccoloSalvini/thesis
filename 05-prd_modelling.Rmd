...
```{r matern}
ggplot(NULL)+
  stat_function(fun = matern, args = list(phi= 0.25, kappa = 1),aes(colour  = "kappa = 1"), size = 1.2, alpha = .6)+
  stat_function(fun = matern, args = list(phi= 0.25, kappa = 1/2),aes(colour = "kappa = 1/2"),size = 1.2, alpha = .6)+
  stat_function(fun = matern, args = list(phi= 0.25, kappa = 3/2),aes(colour = "kappa = 3/2"),size = 1.2, alpha = .6)+
  stat_function(fun = matern, args = list(phi= 0.25, kappa = 80),aes(colour = "kappa = 80"),size = 1.2, alpha = .6)+
  labs(
    x = TeX("distance ($\\Delta_{i\\,j}$)"),
    y = "covariance"
  )+
  scale_colour_brewer(TeX("$Values \\, of \\, \\kappa$")) +
  # scale_colour_viridis("Legend Title", discrete = T) +
  # scale_colour_manual("Legend title", values = c("#56B4E9", "#009E73","#F0E442", "#0072B2")) +
  theme_nicco()
...
```{r expprep}
dataset = faraway::chredlin
formula = involact ~ race + fire + theft + age + log(income)
model = inla(formula, family="gaussian", data=dataset, control.compute=list(cpo=TRUE))
```
...
Then the resulting LPML is: `r sum(log(model$cpo$cpo))`
...
```{r pitcpo, fig.cap="Left: histogram of cross-validated PIT, Right: QQ plot for Unif(0,1)"}
hist = ggplot(data = NULL, aes(x = model$cpo$pit)) + 
...
qq = ggplot(data = NULL, aes(sample = model$cpo$pit)) +  
  stat_qq(distribution = qunif) +
  stat_qq_line(distribution = qunif, colour = "lightblue",alpha=.7, size = 1) +
...
```{r priorfun, fig.cap="PC priors for the precision by varying alpha values and fixing $U$"}
norm_vec = rnorm(20, mean = 40, sd = 2)
sdres = sd(norm_vec)
...
ggplot(NULL)+
  stat_function(fun = pc.prior, args = list(alpha = 0.1, u = 3*sdres), aes(colour  = "alpha = 0.1"), size = 1)+
  stat_function(fun = pc.prior, args = list(alpha = 0.01, u = 3*sdres), aes(colour = "alpha = 0.01"), size = 1)+
  stat_function(fun = pc.prior, args = list(alpha = 1, u = 3*sdres), aes(colour = "alpha = 1"), size = 1)+
  stat_function(fun = pc.prior, args = list(alpha = 0.001, u = 3*sdres), aes(colour = "alpha = 0.001"), size = 1) +
  # stat_function(fun = dgamma, args = list(shape = 1, rate =  0.76), colour = "black", size = 1.2, geom = "point")+
  labs(
    x = TeX("Precision ($\\Delta_{i\\,j\\,}$)"),
    y = "Density"
  )+
  scale_colour_brewer(TeX("$Values \\, of\\, \\alpha$")) +
  # scale_colour_manual("Legend title", values = c("red", "blue", "green", "orange")) +
  theme_nicco()
